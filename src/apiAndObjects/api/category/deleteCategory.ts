...
// Note: just for clarity, this class is implemented but not being used currently ("Category" deletion not allowed nor from DataProduct nor from main "Category" section which has 'admin-only' permission)
export class DeleteCategory extends Endpoint<CategoryDataSource, CategoryType, CategoryDataSource> {
  ...
  protected callLive(body: CategoryType): Promise<CategoryDataSource> {
    ...
    const callResponsePromise = this.apiCaller.doCall(['category'], RequestMethod.DELETE, undefined, body, headers);
